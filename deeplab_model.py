...
def resnet_v2_f(inputs,
  ...
  with variable_scope.variable_scope(
    ...
    with arg_scope(
        [layers_lib.conv2d, resnet_v2.bottleneck, resnet_utils.stack_blocks_dense],
...
def resnet_v2_50(inputs,
    ...
    blocks = [
        resnet_v2.resnet_v2_block('block4', base_depth=512, num_units=1, stride=1),
    ]
    return resnet_v2_f(
        inputs,
        blocks,
...
def deeplab_v3_generator(num_classes,
  ...
  def model(inputs, is_training):
    ...
    if is_training and pre_trained_model:
...
def deeplabv3_model_fn(features, labels, mode, params):
  ...
  def compute_mean_iou(total_cm, name='mean_iou'):
    ...
    for i in range(params['num_classes']):
      # alphabet_symbol = preprocessing.ALPHABET_DICT[i]
      # if alphabet_symbol == '/':
      #   alphabet_symbol = 'background'
      # name = 'train_iou_class_{}'.format(alphabet_symbol)
      name = 'train_iou_class_{}'.format(i)
      tf.identity(iou[i], name=name)
      tf.summary.scalar(name, iou[i])
